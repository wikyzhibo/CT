...
- A 方案多路线批量训练 + 评估（固定并发）:
  ...
  - 可选参数: `--retry`, `--compute-device`, `--rollout-n-envs`, `--lite`
  ...
  - 统一训练时 `solutions.A.ppo_trainer` 不打印训练配置和逐 batch 指标；`validate_all_routes` 在单行内动态刷新训练进度条，评估结束后追加 `eval_pass=T/F`；最终汇总写入 `results/training_logs/validate_all_routes_summary.json`
  - 轻量模式（`--lite`）会关闭甘特图与 `training_metrics_plot` 绘制，并在每条路线评估结束打印 `<route_name> [<profile>] [########################] eval_pass=T/F`
- 导出推理序列:
...
10. `validate_all_routes --lite` 只关闭绘图（甘特图与 `training_metrics_plot`），不改变 summary 表头、summary JSON 字段和评估序列导出。
...
- 正例:
  ...
  - A 方案多路线轻量模式: `python -m solutions.A.eval.validate_all_routes --rollout-n-envs 4 --lite`
- 反例:
...
- `validate_all_routes` 的评估成功判定固定为 `eval_summary["finished"]`；路线级输出中的 `eval_pass=T/F` 仅基于该布尔值。
...
## Change Notes
- 2026-04-05: `solutions/A/eval/validate_all_routes.py` 新增 `--lite`/`lite` 轻量模式：训练与评估禁用甘特图、训练禁用 `training_metrics_plot`，并在每条路线同一行动态刷新训练进度条，评估结束后输出 `<route_name> [<profile>] [########################] eval_pass=T/F`；`_format_summary_table` 与 summary JSON 字段不变。`solutions/A/ppo_trainer.py` 新增 `draw_training_metrics_plot`、`draw_gantt`、`show_batch_progress` 与 `batch_progress_callback` 开关并透传到并发/单动作共用后处理链路。
