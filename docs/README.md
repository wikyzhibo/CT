...
## Change Notes
- 2026-04-05: `solutions/A/eval/validate_all_routes.py` / `solutions/A/ppo_trainer.py` / `training/training-guide.md` / `README.md` / `CHANGELOG.md`：新增 `validate_all_routes --lite` 轻量模式；关闭甘特图与 `training_metrics_plot` 绘制；每条路线评估后输出 `route=<route> [progress] mode=<profile> eval_finish@W<eval_n_wafer>=YES|NO`；最终汇总表与 summary JSON 字段保持不变。
