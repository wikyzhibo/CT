...
## Change Notes
- 2026-04-02: `training/training-guide.md` / `CHANGELOG.md`：新增 `solutions.A.eval.validate_all_routes` 多路线并发训练/评估入口；训练档位固定读取 `config/training/simple.yaml` / `medium.yaml` / `promax.yaml`；统一训练期间 `ppo_trainer` 仅显示单个 batch 进度条；见 `CHANGELOG.md`。
