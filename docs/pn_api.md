...
- **A 方案 `ClusterTool` 甘特**：`render_gantt(out_path, title_suffix=None)` **Does**：直接读取评估模式下 `_fire` 记录的腔室区间（`t_*` 入腔、`u_*` 出腔、`swap` 先关后开），并按每片 wafer 的阶段实际工时生成 `proc_end`；仅保留 `routes.<single_route_name>.route_stage` 声明泳道，未声明腔室记录直接忽略；懒加载调用 `visualization.plot.plot_gantt_hatched_residence`（`no_arm=True`、`policy=2`）。stage 映射规则为：**仅**使用 `route_stage`。若当前路线缺失或留空 `route_stage`，`render_gantt` **抛出** `ValueError`。若 `out_path` 以 `.png` 结尾，会先去掉扩展名再交给绘图函数；最终文件名与该函数约定一致（`{base}{policy}_job{n}.png`）。若评估记录中无腔室占用条，**抛出** `ValueError`。**Does NOT**：训练模式不记录该轨迹；不恢复 `_chamber_timeline` / `_chamber_active`；不从 `fire_log` 重建甘特；不绘制机械手泳道（`no_arm=True`）。
...
- `get_action_mask(wait_action_start=None, n_actions=None, concurrent=None)`：`concurrent` 为 `None` 时跟随构造参数 `concurrent`。`concurrent=False` 时返回完整离散动作掩码 `np.ndarray`（`transition + wait`）。`concurrent=True` 时返回 `(mask_tm1, mask_tm2, mask_tm3)` 三段局部布尔向量（末维为各自 WAIT，恒为可执行）。使能与 wait 规则在完整掩码上统一完成后再投影到 TM。已移除 `get_enable_t()`。补充：`concurrent=True` 时，同一次调用在完整掩码上还会写入 `_cached_auto_tm1_action`；`ClusterTool._pick_tm1_from_mask` 中 TM1 持片时的 `t_TM1_*` 与 mask 使用同一套并行选机/门控，`takt_policy=shared` 且含 `ratio` 时多 `u_LP*_TM1` 同时可行则优先与 `_resolve_required_release_type_for_entry_heads()` 一致的队首 `route_type`（见 `docs/continuous-model/pn-single.md` 规则 39）。
