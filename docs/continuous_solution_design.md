...
- 单设备 `u_LP` 新增“触发间隔追责联动”机制：在 `blame_release_violations()` 中，针对命中追责的 `u_LP` 记录其与前一条 `u_LP` 的时间间隔（秒），按 batch 汇总后执行异常值剔除（偏离均值超过 50%）与上限截断（500s）得到 batch 平均值；当 batch 有命中样本时写入历史队列，当 batch 无命中样本时对历史队列整体乘以 `0.95` 衰减。历史队列仅保留最近 4 个 batch，并按新到旧权重 `4:3:2:1` 计算加权平均，作为 `u_LP` 强制发射间隔；`get_enable_t()` 在 Stage2 中对 `u_LP` 做使能拦截（未达到间隔则禁用）。该限制支持配置开关 `limit_start`（`data/petri_configs/env_config.py`），关闭后仍记录间隔统计，但不对 `u_LP` 使能进行节流限制。
- 单设备 `u_LP` 节拍支持配置持久化：`PetriEnvConfig` 新增 `lp_forced_emit_interval`，`ClusterTool` 初始化会读取该值作为起始强制间隔；当该值大于 0 时，会用该值预填满历史队列（长度 4），避免加载后前几个 batch 因历史不足导致节拍快速失效。`train_single.py` 在训练结束后将“best reward 对应节拍”写回 `data/petri_configs/single.json` 或 `data/petri_configs/cascade.json`（不是最后一个 batch 的节拍）。因此 checkpoint 续训与 `export_inference_sequence.py` 评估模式均可自动加载最近一次最佳模型对应的节拍值。
