...
- 单设备 `u_LP` 新增“触发间隔追责联动”机制：在 `blame_release_violations()` 中，针对命中追责的 `u_LP` 记录其与前一条 `u_LP` 的时间间隔（秒），按 batch 汇总后执行异常值剔除（偏离均值超过 50%）与上限截断（500s）得到 batch 平均值；仅当 batch 存在命中样本时才写入历史队列。历史队列仅保留最近 4 个 batch，并按新到旧权重 `4:3:2:1` 计算加权平均，作为 `u_LP` 强制发射间隔；`get_enable_t()` 在 Stage2 中对 `u_LP` 做使能拦截（未达到间隔则禁用）。该限制支持配置开关 `limit_start`（`data/petri_configs/env_config.py`），关闭后仍记录间隔统计，但不对 `u_LP` 使能进行节流限制。
