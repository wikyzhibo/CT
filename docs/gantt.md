...
1. **入腔**：`t_*` 发射后（`t2`）若目标腔室在 `route_stage` 中，立即记录占用开始，并写入该 wafer 当前阶段实际 `proc_time`。
2. **出腔**：`u_*` 发射前（`t1`）若源腔室在 `route_stage` 中，立即闭合该 wafer 占用区间。
3. **swap**：同一步内对同腔室“先闭合旧片，再开启新片”；关闭时刻取 `t1`，开启时刻取 `t2`。
4. **进行中区间**：`render_gantt` 调用时仍在腔内的 wafer 以当前 `self.time` 作为 `end` 闭合。
