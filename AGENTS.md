...
## 8. 子智能体（Sub-agent）协作规范

当主 Agent 将任务拆分并交给子智能体处理时，子智能体必须遵守与主 Agent 相同的项目规则，不得绕过 `/docs`、约定或文档更新要求。
...
- 始终使用简体中文输出
- 在处理仓库相关任务前，优先访问 `/docs`
- 遵守渐进式读取（Progressive Reading）协议
- 遵守上下文压缩规则，避免大段复制文档或代码
- 对任何功能变更，必须同步更新相关文档
- 输出必须包含变更摘要、文档改动、验证方式、风险说明（如适用）

### 8.2 适用场景

仅当任务可被明确拆分，且子任务具备清晰输入与边界时，才允许创建子智能体，例如：
...
禁止为了“扩大搜索范围”而无边界地并行读取大量代码或全文档。

### 8.3 子智能体的最小输入

主 Agent 在调用子智能体时，必须提供最小必要上下文，格式建议为：

- Task: 子任务目标
- Scope: 允许读取的目录 / 文件 / 文档范围
- Docs to consult: 优先查阅的 docs 路径
- Constraints: 当前已知约束
- Expected output: 需要返回的结果格式

禁止直接向子智能体转发大段代码、整篇文档或未经压缩的上下文。

### 8.4 子智能体的读取限制

子智能体必须遵守“Just Enough”原则：

1. 先读取指定 docs 的 Abstract / TL;DR / 标题结构
2. 仅在确认相关后，继续读取最小必要章节
3. 只有在 docs 不足以回答问题时，才最小范围读取代码
4. 不得全文通读无关文档，不得无边界扫描整个仓库

### 8.5 子智能体返回格式（强制）

子智能体返回内容必须压缩为以下结构，禁止粘贴大段原文：
...
- Derived constraints:
  - 与当前子任务直接相关的约束
- Suggested action:
  - 建议修改点 / 风险 / 待主 Agent 决策项

如子任务涉及功能行为变化，还必须额外说明：
...
主 Agent 不得直接采纳子智能体输出而不做校验。主 Agent 必须：

- 检查子智能体是否遵守 docs-first
- 检查结论是否有明确 DocRef
- 合并多个子智能体结果时消除冲突
- 在最终交付中统一补全文档改动与验证说明
...
如果子智能体结论与主 Agent、其他子智能体或代码行为冲突，按以下优先级处理：
...
若冲突仍无法消除，必须在最终输出中明确标记，不得静默选择其一。
