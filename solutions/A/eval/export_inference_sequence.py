...
`actions: [动作名, "WAIT", "WAIT"]`。

`--concurrent` 时在 `Env_PN_Concurrent` 上加载 `DualHeadPolicyNet`（仅 TM2/TM3 策略头）；TM1 由 `ClusterTool`
内部规则（`get_action_mask` → `_cached_auto_tm1_action`）在每步 `step` 中自动执行。导出 JSON 每步仍含
`actions: [tm1名, tm2名, tm3名]` 与 `action_tm*`，其中 TM1 为当步规则解码结果而非策略输出。
...
def _build_concurrent_policy(
    ...
    if n_actions_tm2 != int(env.n_actions_tm2) or n_actions_tm3 != int(env.n_actions_tm3):
        raise ValueError(
            f"权重动作维 ({n_actions_tm2},{n_actions_tm3}) 与 Env "
            f"({env.n_actions_tm2},{env.n_actions_tm3}) 不一致"
        )
...
def _decode_tm1_rule_export(env: Env_PN_Concurrent, cached_t: int | None) -> tuple[str, int]:
    """本步规则选中的 TM1：`(显示名, 局部 action_tm1 下标)`。`cached_t is None` 表示本步无 TM1 点火（WAIT）。"""
    if cached_t is None:
        return "WAIT", int(env.tm1_wait_action)
    t_idx = int(cached_t)
    for i, tid in enumerate(env.tm1_transition_indices):
        if int(tid) == t_idx:
            return str(env.net.id2t_name[t_idx]), i
    raise RuntimeError(f"TM1 缓存变迁 {t_idx} 不在 tm1_transition_indices 中")


def _decode_tm2_tm3_names(env: Env_PN_Concurrent, a_tm2: int, a_tm3: int) -> tuple[str, str]:
...
def _rollout_concurrent_sequence(
    ...
    with torch.no_grad():
        for step in range(1, MAX_STEPS + 1):
            ...
            cached_t = getattr(env.net, "_cached_auto_tm1_action", None)
            n1, i1 = _decode_tm1_rule_export(env, cached_t)
            n2, n3 = _decode_tm2_tm3_names(env, i2, i3)

            step_td = td.clone()
            step_td["action_tm1"] = torch.tensor(int(env.tm1_wait_action), dtype=torch.int64)
...
def main() -> None:
    ...
    parser.add_argument(
        "--concurrent",
        action="store_true",
        help="使用 DualHeadPolicyNet（Env_PN_Concurrent，仅 TM2/TM3 策略；TM1 规则自动）；默认级联 MaskedPolicyHead",
    )
    parser.add_argument(
        "--retry",
        type=int,
