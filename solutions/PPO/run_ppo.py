...
def create_env(device, training_phase: int = 1, enable_turbo: bool = True):
    """
    创建训练和评估环境
    ...
    Args:
        ...
        enable_turbo: 是否启用 turbo 加速模式
    
    Returns:
        (train_env, eval_env): 训练和评估环境
    """
    base_env1 = Env_PN(device=device, training_phase=training_phase, enable_turbo=enable_turbo)
    base_env2 = Env_PN(device=device, training_phase=training_phase, enable_turbo=enable_turbo)
...
if __name__ == "__main__":
    ...
    train_env, eval_env = create_env(device, training_phase=2, enable_turbo=True)
